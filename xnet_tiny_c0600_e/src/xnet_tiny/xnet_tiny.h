...
#define XARP_ENTRY_RESOLVING	    1               // ARP表项正在解析
#define XARP_ENTRY_OK		        2               // ARP表项解析成功
...
int xip_out(uint8_t protocol, xipaddr_t* dest_ip, xnet_packet_t * packet);
...
typedef struct _xtcp_buf_t {
    uint16_t data_count, wait_send_count;       // 总的数据量+未发送的数据量
...
#define XTCP_CONNECT_TMO                (20 * 1000)     // TCP空闲超时
#define XTCP_RESEND_TMO                 (1 * 1000)      // TCP重发超时
#define XTCP_RESEND_RETRY_CNT           4               // 最多重发次数

#define XTCP_TIMER_PERIOD               500             // TCP扫描的时间间隔
...
struct _xtcp_t {
    ...
    uint16_t connect_tmo;               // 连接超时，即无通信多久自动断开连接
    uint16_t resend_tmo;                // 重发超时，即未收到ack后，多久重发数据
    uint16_t resend_retry_cnt;          // 重发重试次数，最多重试几次重发
};
